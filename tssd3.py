...
# -----------------------------
# SD3 schedule (FlowMatchEulerDiscreteScheduler-style)
# -----------------------------
def make_sd3_t_grid(
    ...
    *,
    ...
    shift: float = 3.0,
    sigma_offset: float = 1e-3,
    renorm: bool = True,
):
    """
    Build t_grid[0..N] from SD3-style sigma schedule:
      sigmas = linspace(1, 1/T, N)
      sigmas = shift*sigmas / (1 + (shift-1)*sigmas)
      sigmas = concat([sigmas, 0])
      t = 1 - sigma
    See FlowMatchEulerDiscreteScheduler.set_timesteps and init logic. :contentReference[oaicite:2]{index=2}
    """
    N = int(denoise_timesteps)
    T = float(num_train_timesteps)

    # base sigmas (descending 1 -> 1/T)
    sigma_max = jnp.array(1.0, dtype=jnp.float32)
    sigma_min = jnp.array(1.0 / T, dtype=jnp.float32)
    sigmas = jnp.linspace(sigma_max, sigma_min, N, dtype=jnp.float32)

    # SD3 shift warp: sigmas = shift*sigmas / (1 + (shift-1)*sigmas)
    s = jnp.array(shift, dtype=jnp.float32)
    sigmas = (s * sigmas) / (1.0 + (s - 1.0) * sigmas)

    # "offset" to push away from extremes (optional)
    if sigma_offset and sigma_offset > 0.0:
        off = jnp.array(sigma_offset, dtype=jnp.float32)
        sigmas = off + (1.0 - 2.0 * off) * sigmas
        sigmas = jnp.clip(sigmas, 0.0, 1.0)

    # append terminal sigma = 0 (as scheduler does)
    sigmas = jnp.concatenate([sigmas, jnp.zeros((1,), dtype=jnp.float32)], axis=0)

    t_raw = 1.0 - sigmas  # increasing-ish

    if renorm:
        # keep endpoints exactly [0,1] even if offset moved them
...
# -----------------------------
# Targets (SD3 schedule + shortcut bootstrap)
# -----------------------------
def get_targets(FLAGS, key, train_state, images, labels, force_t=-1, force_dt=-1):
    ...
    # --- SD3 t-grid (precomputed constants inside jit is fine) ---
    ...
    # 2) =========== Sample t (bootstrap) from SD3 grid, aligned to dt_base. ============
    ...
    # actual "half-step" in t-space under SD3 schedule
    ...
    # 4) =========== Generate Flow-Matching Targets (t from SD3 grid). ============
    ...
    # avoid referencing v_b1/v_b2 if CFG path didn't define them in Python scope
    # (but in JAX both branches exist; here just best-effort debug stats)
