...
def make_sin_t_grid(
    ...
    *,
    ...
    """
    Make a monotone time grid t_grid of length N+1 using:
        x = i/N
        x_tilde = eps + (1 - 2*eps) * x    (avoid endpoints)
        t_raw = sin(pi/2 * x_tilde)        (ease-out: fast early, slow late)
        t = renorm(t_raw) -> [0,1]
    ...
    t_raw = jnp.sin((jnp.pi / 2.0) * x_tilde).astype(dtype)
