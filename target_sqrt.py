...
def make_sqrt_t_grid(
    ...
    *,
    ...
    """
    Make a monotone time grid t_grid of length N+1 using Circular Ease-Out (Sqrt based):
        x = i/N
        x_tilde = eps + (1 - 2*eps) * x    (avoid endpoints)
        t_raw = sqrt(1 - (1 - x_tilde)^2)  (Circular: Extremely fast start, flat end)
        t = renorm(t_raw) -> [0,1]
    ...
    # --- THAY ĐỔI CHÍNH Ở ĐÂY: Hàm Căn (Circular Ease Out) ---
    # f(t) = sqrt(1 - (1-t)^2)
    # Đồ thị là 1/4 hình tròn. Đạo hàm giảm từ vô cùng về 0.
    t_raw = jnp.sqrt(1.0 - (1.0 - x_tilde) ** 2).astype(dtype)
...
def get_targets(FLAGS, key, train_state, images, labels, force_t=-1, force_dt=-1):
    ...
    # --- SQRT t-grid setup ---
    # Sử dụng .get() để fallback về default nếu config chưa có key 'sqrt_*'
    ...
    # 2) =========== Sample t (bootstrap) from SQRT grid, aligned to dt_base. ============
    ...
    # actual "half-step" in t-space under SQRT schedule
    ...
    # 4) =========== Generate Flow-Matching Targets (t from SQRT grid). ============
