...
def _hard_cluster_sums(
    ...
    for sl in chunk_slices(n, chunk_size):
        ...
        sums += np.vstack([np.sum(xb[nearest == j], axis=0, dtype=np.float64) for j in range(k)])
        sums2 += np.vstack([np.sum((xb[nearest == j] ** 2), axis=0, dtype=np.float64) for j in range(k)])
...
def _params_from_hard_assignments(
    ...
    var = np.broadcast_to(np.var(x, axis=0).astype(np.float32), mu.shape).copy()
    nonempty = counts > 0
    if np.any(nonempty):
...
def fit_diag_gmm(
    ...
    best = None
    restart_traces = []
    resolved_init_strategy = _normalize_init_strategy(init_strategy, use_kmeanspp)
    for restart in range(max(int(em_restarts), 1)):
        rng = np.random.default_rng(int(seed) + restart * 1009)
        pi, mu, var, restart_init_strategy = _initial_params(
            x,
            num_modes,
            rng,
            var_floor,
        ...
        for it in range(int(em_iters)):
            ...
            trace_entry = {
                "restart": int(restart),
                "iter": int(it),
                "init_strategy": str(restart_init_strategy),
        ...
        candidate = {
            ...
            "trace": trace,
            "restart": restart,
            "init_strategy": restart_init_strategy,
            "init_warmup_iters": int(init_warmup_iters),
            "init_pca_dims": int(init_pca_dims),
            "init_pca_max_samples": int(init_pca_max_samples),
        }
    ...
    best["init_strategy"] = resolved_init_strategy
    best["init_warmup_iters"] = int(init_warmup_iters)
    best["init_pca_dims"] = int(init_pca_dims)
    best["init_pca_max_samples"] = int(init_pca_max_samples)
    return best
