...
The DiT attention block supports `--model.attn_impl manual`, `--model.attn_impl xla`, and the experimental TPU-only `--model.attn_impl pallas_flash`. This branch defaults to `pallas_flash`, using `jax.experimental.pallas.ops.tpu.flash_attention` inside `shard_map` with the original repo's `1 / head_dim` scaling for checkpoint continuity. Use `xla` or `manual` when running outside TPU or when comparing against the previous attention behavior.
