...
The mix/continue mesh [configs/gmm_tide_fm_mix_continue12_grid.json](configs/gmm_tide_fm_mix_continue12_grid.json) takes the top four finished GMM-TIDE runs and tests three GMM preparation variants for each: mixed GMM fit without continuation, mixed fit with 10 warm-start EM iterations, and x1-only fit with 10 warm-start EM iterations. The four original runs remain the baseline for the no-mix/no-continue setting.

To resume a Kaggle TIDE run, add `resume_kernel_ref` to a submit-grid job, for example `owner/kernel-slug`. The rendered private notebook downloads or mounts that previous notebook output, copies the full output tree back under `/kaggle/working` by default, then copies `gmm_stats.npz` and `gmm_router.pkl` into the new working run directory, finds the latest checkpoint under `ckpts/`, and calls `train.py --load_dir <checkpoint> --reset_step_on_load 0`. The same notebook still downloads the CelebA-HQ dataset and merges dataset `data/` into the repo before training. Optional resume fields are `resume_run_name`, `resume_checkpoint_step`, `resume_download_output`, `resume_copy_full_output`, `resume_copy_to`, `resume_overwrite_code`, `resume_reuse_gmm_router`, and `reset_step_on_load`. By default `resume_overwrite_code` is false, so a previous `shortcut-models/` output is not copied over the freshly checked-out commit; set it true only when intentionally running the code bundled with the old output. When `resume_download_output` is true, the staged notebook uses the pushed account's injected `kaggle.json` to run `kaggle kernels output`, so attaching the previous kernel as a Kaggle kernel source is not required; the temporary Kaggle credential directory is removed after the output download. Generated notebooks also print the tail of redirected stdout/stderr files when `uv sync`, GMM prep, router distillation, or FM training fails, so Kaggle logs contain the real failing stack trace even when output files are not published after an errored notebook.
