...
To resume a Kaggle TIDE run, add `resume_kernel_ref` to a submit-grid job, for example `owner/kernel-slug`. The rendered private notebook downloads or mounts that previous notebook output, copies `gmm_stats.npz` and `gmm_router.pkl` into the new working run directory, finds the latest checkpoint under `ckpts/`, then calls `train.py --load_dir <checkpoint> --reset_step_on_load 0`. The same notebook still downloads the CelebA-HQ dataset and merges dataset `data/` into the repo before training. Optional resume fields are `resume_run_name`, `resume_checkpoint_step`, `resume_download_output`, `resume_reuse_gmm_router`, and `reset_step_on_load`.
