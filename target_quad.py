...
def make_quad_t_grid(
    ...
    *,
    ...
    """
    Make a monotone time grid t_grid of length N+1 using Quadratic Ease-Out:
        x = i/N
        x_tilde = eps + (1 - 2*eps) * x    (avoid endpoints)
        t_raw = 1 - (1 - x_tilde)^2        (Quadratic: fast early, slow late)
        t = renorm(t_raw) -> [0,1]
    ...
    # --- THAY ĐỔI CHÍNH Ở ĐÂY: Hàm Bậc 2 (Quadratic) ---
    # f(t) = 1 - (1-t)^2
    t_raw = (1.0 - (1.0 - x_tilde) ** 2).astype(dtype)
...
def get_targets(FLAGS, key, train_state, images, labels, force_t=-1, force_dt=-1):
    ...
    # --- QUAD t-grid setup ---
    # Sử dụng .get() để fallback về default nếu config chưa có key 'quad_*'
    ...
    # 2) =========== Sample t (bootstrap) from QUAD grid, aligned to dt_base. ============
    ...
    # actual "half-step" in t-space under QUAD schedule
    ...
    # 4) =========== Generate Flow-Matching Targets (t from QUAD grid). ============
